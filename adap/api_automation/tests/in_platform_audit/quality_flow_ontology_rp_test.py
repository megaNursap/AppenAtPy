...
def test_get_answer_distribution(rp):
    ...
    rp_res.assert_response_status(200)
    assert rp_res.json_response[0]['name'] == 'annotation'
    assert rp_res.json_response[0]['aggregations_distribution']['type'] == 'ontology'
    time.sleep(5)


@allure.parent_suite('jobs/{job_id}/audit/units/search')
def test_search_units_for_audit_all(rp):
    payload_all_units = {"filters": {"question": {"name": "annotation", "values": []}}}
    ...
    assert all_units['units'][4]['audited'] == True
...
def test_search_units_audited_filter(rp):
    payload_not_audited = {"filters": {"question": {"name": "annotation", "values": []}}, "audited": True, "page": 1,
                           "per_page": 100}
    ...
    assert all_units['units'][0].get('unit_id')
    assert all_units['units'][0]['audited'] == True
    assert all_units['units'][0].get('data')
    assert all_units['units'][0].get('fields')
...
def test_get_audit_info_for_unit(rp):
    payload_audited = {"filters": {"question": {"name": "annotation", "values": []}},
                       "filter_data_columns": ["image_url"], "page": 1, "per_page": 100}
    ...
    assert res.json_response['nodes']['nodes'][0]['tag'] == 'image_segmentation'


@allure.parent_suite('jobs/{job_id}/audit/units/{unit_id}')
def test_add_audit_info_for_unit_correct(rp):
    payload_find_units = {"filters": {"question": {"name": "annotation", "values": []}}, "page": 1, "audited": True,
                          "per_page": 10}
    ...
    reason = "Reason by API" + str(_index)
    random_unit = all_units[_index]['unit_id']
    payload = {
        "fields":
            {
    ...
    assert res.json_response['fields']['annotation']['audit'] == {
        "correct": True,
        "answers": [],
        "reason": reason,
        "custom_answers": [],
