...
def answer_questions_on_page(driver, tq_dict):
    questions_on_page = driver.find_elements('xpath','//div[@class="cml jsawesome"]')
    for q in questions_on_page:
        time.sleep(1)
        text = q.find_element('xpath',".//div[@class='html-element-wrapper']//span").text
...
def get_judgment_for_job(link=None, users=None):
    for user in users:
