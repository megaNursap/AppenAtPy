...
def taxonomy_job(rp_adap):
    rp_adap.upload_taxonomy_file(TAXONOMY_TEST_DATA, JOB_ID, TEAM_ID, CUSTOM_UUID, name='taxonomy_2')


def test_get_taxonomy_url(rp_adap, taxonomy_job):
    """
    Verify GET /v1/taxonomy/%s/url?teamId=%s is successful with valid parameters
    """

    res = rp_adap.get_taxonomy_url(JOB_ID, TEAM_ID)
...
def test_get_without_team_id_parameter_taxonomy_url_rp(rp_adap, taxonomy_job):
    """
    Verify GET /v1/taxonomy/%s/url? is error without TeamId
    """
...
def test_get_with_incorrect_team_id_parameter_taxonomy_url_rp(rp_adap, taxonomy_job):
    """
    Verify GET /v1/taxonomy/%s/url?teamId=%s is error with incorrect TeamId value
    """
...
def test_get_with_incorrect_job_id_parameter_taxonomy_url_rp(rp_adap, taxonomy_job):
    """
    Verify GET /v1/taxonomy/%s/url?teamId=%s is error with incorrect JobId value
    """
