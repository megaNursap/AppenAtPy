...
class CreateWorkflow(TaskSet):
    ...
    def create_wf(self):
        ...
        file_name = '500Rows.csv'
        storage_key = '00000000-0000-0000-0000-000000000002/%s' % file_name
        self.upload_data(file_name, storage_key)
        time.sleep(20)

        file_name = '5KRows.csv'
        storage_key = '00000000-0000-0000-0000-000000000002/%s' % file_name
        self.upload_data(file_name, storage_key)
        time.sleep(20)

        file_name = '10KRows.csv'
        storage_key = '00000000-0000-0000-0000-000000000002/%s' % file_name
        self.upload_data(file_name, storage_key)
        time.sleep(20)

        file_name = '20KRows.csv'
        storage_key = '00000000-0000-0000-0000-000000000002/%s' % file_name
        self.upload_data(file_name, storage_key)
        time.sleep(20)
    ...
    def copy_job(self, copy_job):
        url ="https://api.%s.cf3.us/v1/jobs/%s/copy.json" %(ENV, copy_job)
        ...
        with self.client.get(url, name="/v1/jobs/[copy_job_id]/copy.json",  params=payload, catch_response=True) as response:
    ...
    def launch_wf(self):
        ...
        querystring = {"user_id": akon_id, "rows": "100"}
        payload = ""
        headers = {
            'cache-control': "no-cache",
        }
        with self.client.post(WF_URL + url, name="/[WF_ID]/launch", data=payload, headers=headers, params=querystring,
                              catch_response=True) as response:
            if response.status_code == 202 and response.json()['status'] == 'running':
                response.success()
    ...
    def get_data_upload(self):
        ...
        while status != 'completed' and status != 'failed' and current_try < max_try:
            ...
            with self.client.get(WF_URL + url, name="/[WF_ID]/data_uploads/[WF_DATA_ID]", data=payload, headers=headers, params=querystring,
                                 catch_response=True) as response:
    ...
    def upload_data(self, file_name, storage_key):
        ...
        with self.client.post(WF_URL + url, name="/%s/data_uploads" % file_name, data=json.dumps(payload), headers=headers, catch_response=True) as response:
    ...
    def filter_rules(self):
        ...
        with self.client.post(rules_url, name="/v1/step_routes/[ROUTE_ID]/filter_rules", data=json.dumps(payload), headers=headers, catch_response=True) as response:
            if response.status_code == 201:
                response.success()
    ...
    def create_route(self):
        ...
        with self.client.post(WF_URL + url, name="/[WF_ID]/steps/[STEP_ID]/routes", data=json.dumps(payload), headers=headers, catch_response=True) as response:
            if response.status_code == 201:
                response.success()
    ...
    def determine_steps(self):
        ...
        for job in (self.JOBS[0], self.JOBS[1]):
            ...
            with self.client.post(WF_URL + url, name="/[WF_ID]/steps",  data=json.dumps(payload), headers=headers,
                                  catch_response=True) as response:
                if response.status_code == 201:
                    response.success()
    ...
    def create_new_wf(self):
        ...
        payload = {
            'name': "Test WF for jobs %s and %s" % (self.JOBS[0], self.JOBS[1]),
