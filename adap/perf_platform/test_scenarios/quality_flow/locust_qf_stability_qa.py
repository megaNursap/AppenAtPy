...
QF_QA_WORKING_SECONDS_PER_TASK_PAGE = int(decouple.config('QF_QA_WORKING_SECONDS_PER_TASK_PAGE', default='1', cast=str))
QF_QA_REVIEW_RESULT_LIST = decouple.config('QF_QA_REVIEW_RESULT_LIST', default='ACCEPTED', cast=str)
QF_QA_ABANDON_PERCENTAGE = int(decouple.config('QF_QA_ABANDON_PERCENTAGE', default='0', cast=str))
    ...
    def simulate_qa_action(self):
        ...
        try:
            _, fetch_info = qfm.logic_distribution_fetch_and_submit(
                ...
                working_seconds_per_task_page=QF_QA_WORKING_SECONDS_PER_TASK_PAGE,
                job_type=JobType.QA,
                review_result_list=list(map(str.strip, QF_QA_REVIEW_RESULT_LIST.split(','))),
                judgment_keys=JUDGMENT_KEYS,
                auto_judgment=None,
                audio_tx_judgment=audio_tx_judgment,
                abandon_percentage=QF_QA_ABANDON_PERCENTAGE,
