...
with se.session(desc, teardown=False) as session_id:
    ...
    se.execute_concurrent(create_job, params=TASK_IDS)
    se.wait(10*60)

    def deploy_locust(TASK_ID):
    ...
    se.execute_concurrent(deploy_locust, params=['1', '2'])
    se.wait(3*60)
    se.execute_concurrent(deploy_locust, params=['3'])
    se.wait(11*60)