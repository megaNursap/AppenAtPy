...
pytestmark = [pytest.mark.regression_ac_project_view, pytest.mark.regression_ac, pytest.mark.ac_ui_new_project,
              pytest.mark.pay_rate]
...
def test_default_pay_rate_view(app, login):
    ...
    if pytest.env == "prod":
        select_registration_process = "Agnew registration"
        select_qualification_process = "Aliso Qualification"
    else:
        select_registration_process = "Acac Register"
        select_qualification_process = "Apple NDA"

    app.ac_project.registration.fill_out_fields(data={
        "Select Registration process": select_registration_process,
        "Select Qualification process": select_qualification_process
    })
    ...
    app.ac_project.recruitment.add_target(language='Portuguese', region='Brazil',
                                          restrict_country='United States of America',
                                          deadline=tomorrows_date, target='12')

    # Adding first second target
    app.ac_project.recruitment.add_target(language='English', region='United States of America',
                                          restrict_country='United States of America',
                                          deadline=tomorrows_date, target='12')
    ...
    app.ac_project.payment.verify_pay_rate_is_present_on_page("Portuguese (Brazil)U.S.A.", "Native or Bilingual", ".",
                                                              ".", ".", "$9.00", ".", )
...
@pytest.mark.dependency(depends=["test_default_pay_rate_view"])
def test_verify_status_draft_view(app, login):
...
@pytest.mark.dependency(depends=["test_verify_status_draft_view"])
def test_finish_project_creation_for_view_and_edit_view(app, login):
...
@pytest.mark.dependency(depends=["test_finish_project_creation_for_view_and_edit_view"])
def test_default_pay_rate_required_fields_in_view_mode_view(app, login):
    app.navigation.switch_to_frame("page-wrapper")
    app.ac_project.click_on_step('Invoice & Payment')
    app.navigation.click_btn('Edit')
    app.navigation.click_btn("Add Pay Rate")
    # assert app.verification.button_is_disable('Save')
    app.navigation.click_btn('Save')
    app.verification.text_present_on_page('Hiring Target is a required field.')


@pytest.mark.dependency(depends=["test_default_pay_rate_required_fields_in_view_mode_view"])
def test_add_default_pay_rate_already_exist_view(app, login):
    app.ac_project.payment.select_default_pay_rates('English (U.S.A.)', 'U.S.A.')
    app.ac_project.payment.select_pay_rate('Contractor', 'CR', 'Hourly', '$10.00', 'AR', action='Save')
...
@pytest.mark.dependency(depends=["test_add_default_pay_rate_already_exist_view"])
def test_add_new_default_pay_rate_in_view_mode_view(app, login):
...
# @pytest.mark.skip(reason='ACE-8868')
@pytest.mark.dependency(depends=["test_finish_project_creation_for_view_and_edit_view"])
def test_copy_default_payrates_already_exist_view(app, login):
    app.ac_project.click_on_step('Invoice & Payment')
    app.navigation.click_btn('Edit')
...
# # @pytest.mark.skip(reason='ACE-8868')
# @pytest.mark.dependency(depends=["test_copy_default_payrates_already_exist_view"])
# def test_copy_default_payrates_successfully_view(app, login):
#     app.ac_project.payment.select_pay_rate('Contractor', 'CR', 'Hourly', '$12.00', 'AK')
...
# def test_delete_default_payrates_view(app, login):
#     app.ac_project.click_on_step('Invoice & Payment')
#     app.navigation.click_btn('Edit')
...
#                                                               input_fluency='Native or Bilingual',
#                                                               input_grp='All users',
#                                                               input_work='Default', input_taskid='.',
#                                                               input_rate='$12.00', state='AK', is_not=False)
