...
pytestmark = [pytest.mark.regression_ac_project_config, pytest.mark.regression_ac, pytest.mark.ac_ui_new_project, pytest.mark.ac_ui_project_invoice]
...
def test_access_control_invoice_config_default(app, invoice_project):
    ...
    assert app.ac_project.payment.get_options_for_dropdown("Invoice type") == [{'name': 'User self-reported', 'title': ''},
                                                                               {'name': 'System generated', 'title': 'Enabled only when “Available Productivity Data” is checked.'},
                                                                               {'name': 'User self-reported and System approved', 'title': 'Enabled only when “Available Productivity Data” is checked.'}]

    assert app.ac_project.payment.auto_approve_is_disable()
    app.navigation.click_btn("Cancel")


@pytest.mark.dependency(depends=["test_access_control_invoice_config_default"])
def test_access_control_invoice_config_productivity_data(app, invoice_project):
...
@pytest.mark.dependency(depends=["test_access_control_invoice_config_productivity_data"])
def test_user_self_reported(app, invoice_project):
    app.navigation.click_btn("Set Template")
    ...
    app.ac_project.payment.save_invoice_config()
    app.verification.text_present_on_page("Project with hourly rate can not have auto approve invoice by units of work.")
...
@pytest.mark.dependency(depends=["test_user_self_reported"])
def test_invoice_system_generated(app, invoice_project):
    ...
    app.ac_project.payment.save_invoice_config()
    app.verification.text_present_on_page("Project with hourly rate can not have auto generate invoice by units of work.")
...
@pytest.mark.dependency(depends=["test_invoice_system_generated"])
def test_invoice_self_report_and_system_generated(app, invoice_project):
    app.ac_project.payment.click_edit_invoice_config()
...
@pytest.mark.dependency(depends=["test_invoice_self_report_and_system_generated"])
def test_invoice_by_task_config(app, invoice_project):
    app.ac_project.payment.fill_out_fields({"Rate Type": "By Task"})

    app.ac_project.payment.click_edit_invoice_config()
    app.ac_project.payment.save_invoice_config()

    app.verification.text_present_on_page("Project with rate type by task must have auto generate invoice by units of work.")
...
def test_invoice_no_access(app_test):
...
@pytest.mark.skip  #Skipped due to Idaptive Issue
def test_create_invoice_config_no_access(app_test):
    user_name = get_user_email('test_qa_user')
    password = get_user_password('test_qa_user')
